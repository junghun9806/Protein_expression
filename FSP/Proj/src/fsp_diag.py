...
def solve_FSP_diag(pmax, mmax, para_list, file_name, data_dir= "data/"): 
    """
...
def generate_P_matrix_FSP(pmax, mmax, eigen_rank, eig_val, eig_vec):
...
def draw_P_hmap_FSP(Pmap, Gmap, eig_value, title, para_list, plot_dir= "plots/"):
...
def do_all_FSP(pmax, mmax, para_list, max_rank, plot_dir, data_dir): 
    rw.make_dir(plot_dir)
    rw.make_dir(data_dir)
    
    file_prefix = f"FSP_Diag_p_{pmax}_m_{mmax}_para_{para_list[0]}_{para_list[1]}_{para_list[2]}"
    
    eig_val_list, eig_vec_list = solve_FSP_diag(pmax, mmax, para_list, file_prefix, data_dir=data_dir)
    for rank in range(max_rank):
        ...
        if rank >= len(eig_val_list) :
            continue
        # 
        prob_eq, prob_g, eig_val = generate_P_matrix_FSP(pmax, mmax, rank, eig_val_list, eig_vec_list) 
        title = file_prefix + f"_rank_{rank}"
        draw_P_hmap_FSP(prob_eq, prob_g, eig_val, title, para_list, plot_dir=plot_dir)
...
if __name__ == "main": 
    ...
    do_all_FSP(pmax, mmax, para_list, max_rank, plot_dir, data_dir)
#